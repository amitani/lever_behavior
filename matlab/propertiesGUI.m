function [hPropsPane,parameters, propsList] = propertiesGUI(hParent, parameters, filename, selectedBranch)
...
% $Revision: 1.15 $  $Date: 2015/03/12 12:37:46 $
  ...
  if nargin < 2
      ...
      if isObj
          ...
          if(~isempty(ancestor(hParent,'figure')) && ~isempty(getappdata(ancestor(hParent,'figure'),'mirror')))
              hPropsPane=hParent;
              parameters=getappdata(ancestor(hPropsPane,'figure'),'mirror');
              propsList=getappdata(ancestor(hPropsPane,'figure'),'propsList');
              return;
          else
  ...
  elseif nargin == 3 || nargin == 4  % check if load or save mode
      if(nargin == 3)
          selectedBranch='';
      end
      mode = parameters;
